...
# ── ECS Security Group (created here to break the rds <-> ecs cycle) ──
...
module "rds" {
  ...
  ecs_security_group_id = aws_security_group.ecs_tasks.id  # no cycle — SG is a local resource
...
module "ecs" {
  ...
  ecs_security_group_id = aws_security_group.ecs_tasks.id  # passed in, not created internally
...
module "cicd" {
  ...
  # Fix #2: use the actual TG names from the ALB module, not hardcoded "blue"/"green"
  blue_target_group_name  = module.alb.blue_target_group_name
  green_target_group_name = module.alb.green_target_group_name
  # Fix #3: pass the ALB arn_suffix for CloudWatch dashboard
